...
def loop_op(builder: "GraphBuilder", node: onnx.NodeProto) -> torch.fx.Node:
    ...
    # Try to get max_trip_count as a constant
    max_iters = None
    if max_trip_count_node is not None:
        ...
        # Check if it's directly in env as a known value
        if max_iters is None and node.input[0] in builder.initializer_map:
            max_iters = int(builder.initializer_map[node.input[0]].item())

    # If we can't determine max_iters at conversion time, use a reasonable default
    ...
    for v in initial_values:
        if v is None:
            ...
            const_name = v.target
            if const_name in builder._constants:
                loop_vars.append(builder._constants[const_name].clone())
            else:
                loop_vars.append(torch.tensor([]))
        else:
            # Try to get from initializer
            found = False
            for inp_name, val in builder.initializer_map.items():
                if inp_name in node.input[2:]:
                    idx = list(node.input[2:]).index(inp_name)
                    if idx < len(loop_vars):
                        continue
                    loop_vars.append(val.clone())
                    found = True
                    break
            if not found:
    ...
    # Ensure loop_vars has the right length
    while len(loop_vars) < num_loop_vars:
        loop_vars.append(torch.tensor([]))
    ...
    for i in range(max_iters):
        ...
        for n in body.node:
            try:
                result = _execute_body_node(n, env, body_initializers)
                if len(n.output) == 1:
                    env[n.output[0]] = result
                else:
                    for k, out_name in enumerate(n.output):
    ...
    for j, val in enumerate(loop_vars):
        if val is not None:
            output_name = f"loop_output_{node.name}_{j}"
            safe_name = output_name.replace(".", "_").replace("/", "_")
            builder._constants[safe_name] = val
    ...
    for j, acc in enumerate(scan_accumulators):
        if acc:
            stacked = torch.stack(acc, dim=0)
            output_name = f"loop_scan_{node.name}_{j}"
            safe_name = output_name.replace(".", "_").replace("/", "_")
            builder._constants[safe_name] = stacked
            fx_node = builder.graph.get_attr(safe_name)
            fx_node.meta["onnx_op_type"] = "Loop"
            all_outputs.append(fx_node)
    ...
    if len(all_outputs) == 1:
        ...
        # Return empty tensor if no outputs
        empty_name = f"loop_empty_{node.name}"
        builder._constants[empty_name] = torch.tensor([])
        return builder.graph.get_attr(empty_name)
    else:
        return builder.call_function(lambda *args: args, args=tuple(all_outputs))
